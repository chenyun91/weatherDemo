...
public class DWebView extends WebView {
            ...
            if (activity != null) {
                switch (msg.what) {
                    ...
                    case LOAD_URL:
                        ...
                        LogUtils.INSTANCE.logI("MyHandler", "handleMessage: +loadUrl");
                        break;
    ...
    class InnerJavascriptInterface {
        ...
        private void PrintDebugInfo(String error) {
            LogUtils.INSTANCE.logD(LOG_TAG, error);
            if (isDebug) {
        ...
        public void hello(String msg) {
            System.out.println("JS调⽤了Android的hello⽅法" + msg);
        }
        ...
        public String call(String methodName, String argStr) {
            LogUtils.INSTANCE.logD(LOG_TAG, "js call native methodName" + methodName);
            LogUtils.INSTANCE.logD(LOG_TAG, "js call native argStr" + argStr);
                    ...
                    method.invoke(jsb, arg, new CompletionHandler() {
                        ...
                        private void complete(Object retValue, boolean complete) {
                            try {
                                JSONObject ret = new JSONObject();
                                ret.put("code", 0);
                                ret.put("data", createResponse(retValue));
    ...
    private void _evaluateJavascript(String script) {
        LogUtils.INSTANCE.logI(LOG_TAG, "_evaluateJavascript: "+script);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
    ...
    private synchronized void dispatchStartupQueue() {
        LogUtils.INSTANCE.logI(LOG_TAG, "dispatchStartupQueue: ");
    ...
    public synchronized <T> void callHandler(String method, Object[] args, final OnReturnValue<T> handler) {
        ...
        LogUtils.INSTANCE.logD(LOG_TAG, "native call js method" + callInfo.toString());
        if (handler != null) {
...
//
    ...
    public void deleteFile(File file) {
        if (file.exists()) {
            ...
            LogUtils.INSTANCE.logE(LOG_TAG, "delete file no exists " + file.getAbsolutePath());
        }
    }
    ...
    public void destroy() {
        ...
        if (mainThreadHandler != null) {
            mainThreadHandler.destroy();
        }
    }

    private JSONObject createResponse(Object jsonObject) throws JSONException {
